...
변경점 (원본 대비):
    ...
    - google.generativeai를 __init__ 내부에서 지연 import
      (Why: local 엔진 사용 시 이 패키지 미설치여도 에러가 나지 않아야 한다)
    - tracker 전역 → self.tracker 인스턴스 변수
    - SAFETY_SETTINGS → 클래스 상수로 이동
    - genai.configure() → __init__ 에서만 호출 (전역 X)
...
"""
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "Gemini 엔진을 사용하려면 google-generativeai 패키지가 필요합니다.\n"
