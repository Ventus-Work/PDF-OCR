"""
test_phase5_unit5_preflight.py — 단위 5 프로덕션 배치 전 사전 점검 (Dry-Run)

실행: python test_phase5_unit5_preflight.py
목적:
    실제 PDF(84개, 5.7MB)에 대한 배치 실행 전에
    환경, 설정, 파일 접근성을 모두 점검하여
...
점검 항목:
    ...
    MOD-4  export_aggregated_excel 임포트 가능 확인
...
# ── 배치 제외 패턴: 표지/목록/물량 집계표는 BOM 도면이 아님 ──
# 이 파일들은 개별 서포트 규격이 아니라 도면 목록이나 집계 시트이므로 제외.
_EXCLUDE_KEYWORDS = [
    "COVER", "DWG LIST", "물량", "SUPPORT_물량"
]

# ── 출력 경로 ──
_OUTPUT_DIR = _HERE / "output" / "unit5_production"
...
def _is_bom_target(path: Path) -> bool:
    """표지·목록·물량 집계 파일을 제외한 실제 BOM 도면만 선별."""
...
def run_preflight():
    print("\n" + "=" * 62)
    print("  단위 5: 프로덕션 배치 사전 점검 (Dry-Run)")
    print("=" * 62 + "\n")
    ...
    cache_dir_ok = config.CACHE_DIR.exists() or True  # 실행 시 자동 생성
    check(f"CACHE_DIR 경로: {config.CACHE_DIR.name}", True,
          str(config.CACHE_DIR))

    print("\n[ENV-3] 출력 디렉토리 쓰기 권한")
    try:
        _OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        test_file = _OUTPUT_DIR / ".write_test"
